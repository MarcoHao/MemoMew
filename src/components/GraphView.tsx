...
const GraphView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const link = svg.append('g').selectAll('line').data(links).join('line')
      .attr('stroke', '#cbd5e1').attr('stroke-width', 1.5).attr('opacity', 0.6);

    const linkLabel = svg.append('g').selectAll('text').data(links).join('text')
      .text((d: any) => d.type).attr('font-size', '10px').attr('fill', '#94a3b8').attr('text-anchor', 'middle');
    ...
    node.append('circle').attr('r', 20).attr('fill', (d: any) => colorScale(d.type) || '#94a3b8')
      .attr('stroke', '#fff').attr('stroke-width', 2);
    node.append('text').text((d: any) => d.name).attr('text-anchor', 'middle').attr('dy', 35)
      .attr('font-size', '11px').attr('fill', '#475569').attr('font-weight', '500');
    node.append('text').text((d: any) => d.type).attr('text-anchor', 'middle').attr('dy', 48)
      .attr('font-size', '9px').attr('fill', '#94a3b8');

    node.on('click', (event, d) => setSelectedNode(d));

    simulation.on('tick', () => {
      link.attr('x1', (d: any) => (d.source as GraphNode).x!).attr('y1', (d: any) => (d.source as GraphNode).y!)
        .attr('x2', (d: any) => (d.target as GraphNode).x!).attr('y2', (d: any) => (d.target as GraphNode).y!);
      linkLabel.attr('x', (d: any) => ((d.source as GraphNode).x! + (d.target as GraphNode).x!) / 2)
